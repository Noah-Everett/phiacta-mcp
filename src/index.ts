...
function buildInstructions(plugins: import("./client.js").PluginInfo[], authenticated: boolean): string {
  ...
  lines.push(
    ...
    "",
    "**After creating entries, wire them up with references.** If a new entry depends on, contains, cites, extends, supports, or contradicts another entry, create a reference: `create_reference` with source `entry_id`, `target_entry_id`, and a `rel` like `uses`, `contains`, `extends`, `supports`, `contradicts`. References turn isolated entries into a knowledge graph — skipping them is the single most common mistake. See `phiacta://docs/references` for relation roles.",
    "",
    "## Updating entries: metadata vs. content",
    "",
    "Different fields use different tools:",
    "",
    "- **Metadata** (`title`, `summary`, `entry_type`, `tags`, `visibility`): use `update_entry` (PATCH). Direct write, no review.",
    "- **Content** is the entry's git repository, not a single string. A markdown entry might be just `.phiacta/content.md`; a LaTeX entry can span many files (`main.tex` + `references.bib` + `figures/...` + `sections/...`); a plain-text entry is `.phiacta/content.txt`. Because content is multi-file in general, it cannot be sent as a `content` field on PATCH — write files directly.",
    "  - **You own the entry**: use `put_entry_file` with the file path. For the main content file the canonical path is `.phiacta/content.<ext>` where `<ext>` matches the entry's `content_format`. Writes directly to `main`.",
    "  - **You don't own the entry**: use `create_edit_proposal` to open a branch + PR with one or more file changes. The owner reviews and merges.",
    "",
    "**`update_entry` cannot change content — sending `content` to PATCH returns 422 with a pointer to both routes above.**",
    "",
    "## Conventions",
    "",
    ...
    "",
    "Each entry is a git repo. The main content file is at `.phiacta/content.<ext>` where `<ext>` is determined by the entry's `content_format` (see `phiacta://docs/content-formats`). All other paths in the repo are yours — figures, code, bibliographies, sub-sections of a LaTeX project, datasets, anything that supports the entry. See `phiacta://docs/files` for what to attach and how to organize it. The identity file `.phiacta/entry.yaml` is immutable.",
    "",
    "## Entity resolve",
    "",

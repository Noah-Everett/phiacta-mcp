...
function buildInstructions(plugins: import("./client.js").PluginInfo[], authenticated: boolean): string {
  ...
  lines.push(
    ...
    "",
    "Two different endpoints, two different things:",
    "",
    "- **Metadata** (`title`, `summary`, `entry_type`, `tags`, `visibility`): use `update_entry` (PATCH). Direct write, no review.",
    "- **Content** (the body of the entry — what readers see): use `create_edit_proposal` (POST /v1/entries/{id}/edits). This opens a branch + PR in the entry's git repo. Owner-only merge. **`update_entry` cannot change content — sending `content` to PATCH returns 422.**",
    "",
    "## Conventions",
    "",
    ...
    "",
    "Each entry's content lives in `.phiacta/content.md` (or `.tex`, `.txt`) in its git repo. To change it, propose an edit via `create_edit_proposal` with that path and the new file contents. The identity file `.phiacta/entry.yaml` is immutable and cannot be modified.",
    "",
    "## Entity resolve",
    "",

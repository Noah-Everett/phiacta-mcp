...
function buildInstructions(plugins: import("./client.js").PluginInfo[], authenticated: boolean): string {
  ...
  lines.push(
    ...
    "- **Content** (the body of the entry — what readers see): depends on whether you own the entry.",
    "  - **You own it**: use `put_entry_file` with path `.phiacta/content.md` (or `.tex`/`.txt` matching the entry's `content_format`). Writes directly to `main`.",
    "  - **You don't own it**: use `create_edit_proposal` to open a branch + PR. The owner reviews and merges.",
    "",
    ...
    "",
    "Each entry's content lives in `.phiacta/content.md` (or `.tex`, `.txt`) in its git repo. See the section above for whether to use `put_entry_file` (owner) or `create_edit_proposal` (non-owner). The identity file `.phiacta/entry.yaml` is immutable and cannot be modified.",
    "",
    "## Entity resolve",
    "",
